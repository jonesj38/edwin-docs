...
- Jobs persist under `~/.edwin/cron/` so restarts don’t lose schedules.
...
edwin cron list
edwin cron run <job-id> --force
edwin cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.edwin/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `edwin cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Edwin defaults to `announce`.
...
- Job store: `~/.edwin/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.edwin/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
edwin cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
edwin cron edit <jobId> --agent ops
edwin cron edit <jobId> --clear-agent
```
...
```bash
edwin system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`edwin system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `EDWIN_SKIP_CRON`.
