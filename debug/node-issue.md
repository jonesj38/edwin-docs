...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Edwin
...
Running Edwin via Node with `tsx` fails at startup with:

```
[edwin] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `edwin status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch edwin.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node edwin.mjs status` works on Node 25.
