...
```bash
edwin tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `edwin tui --deliver`
...
- Check the Gateway logs: `edwin logs --follow`.
- Confirm the agent can run: `edwin status` and `edwin models status`.
...
- No agents in picker: check `edwin agents list` and your routing config.
