...
Deploy Edwin on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/jonesj38/edwin)
...
repository configures everything needed to run Edwin:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: EDWIN_STATE_DIR
        value: /data/.edwin
      - key: EDWIN_WORKSPACE_DIR
        value: /data/workspace
      - key: EDWIN_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/edwin`.
...
If you use the original Edwin repository, Render will not auto-deploy your Edwin. To update it, run a manual Blueprint sync from the dashboard.
...
For Edwin, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Edwin host.
