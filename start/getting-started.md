...
Fastest chat: open the Control UI (no channel setup needed). Run `edwin dashboard`
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `edwin configure --section web` (stores `tools.web.search.apiKey`).
...
If you don’t have a global install yet, run the onboarding step via `pnpm edwin ...` from the repo.
...
Tip: `edwin status --all` is the best pasteable, read-only debug report.
Health probes: `edwin health` (or `edwin status --deep`) asks the running gateway for a health snapshot.
