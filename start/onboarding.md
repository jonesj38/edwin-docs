...
- Writes credentials to `~/.edwin/credentials/oauth.json`
...
The app can install the global `edwin` CLI via npm/pnpm so terminal
...
On the first agent run, Edwin bootstraps a workspace (default `~/.edwin/workspace`):
...
```bash
edwin webhooks gmail setup --account you@gmail.com
```
