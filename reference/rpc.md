...
Edwin integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Edwin owns lifecycle when `channels.signal.autoStart=true`.
...
- Edwin spawns `imsg rpc` as a child process (legacy iMessage integration).
