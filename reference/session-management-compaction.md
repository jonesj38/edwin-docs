...
This document explains how Edwin manages sessions end-to-end:
...
Edwin is designed around a single **Gateway process** that owns session state.

- UIs (Edwin Desktop, web Control UI, TUI) should query the Gateway for session lists and token counts.
...
Edwin persists sessions in two layers:
...
- Transcripts: `~/.edwin/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Edwin resolves these via `src/config/sessions.ts`.
...
Edwin intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Edwin consumes the events, but Pi decides when to compact).
...
Edwin also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Edwin bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Edwin leaves it alone.
...
Edwin supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Edwin strips/suppresses this in the delivery layer.

As of `2026.1.10`, Edwin also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Edwin uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Edwin’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `edwin status`.
