...
Edwin features that can generate provider usage or paid API calls.
...
- `edwin status --usage` and `edwin channels list` show provider **usage windows**
...
Edwin can pick up credentials from:
...
`edwin models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
