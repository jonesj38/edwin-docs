...
- Split execution into a **headless runner service** with optional UI (Edwin Desktop) via local IPC.
...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.edwin/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.edwin/node.json` (node id + pairing token).
- **macOS exec host:** run `system.run` inside the Edwin Desktop; node host service forwards requests over local IPC.
...
Path: `~/.edwin/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.edwin/exec-approvals.sock",
...
## UI integration (Edwin Desktop)

### IPC

- Unix socket at `~/.edwin/exec-approvals.sock` (0600).
...
### Ask flow (Edwin Desktop exec host)
...
- Add Unix socket prompt bridge to Edwin Desktop UI.
- Wire `askFallback`.
