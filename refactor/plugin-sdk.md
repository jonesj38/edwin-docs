...
- Publish as `edwin/plugin-sdk` (or export from core under `edwin/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `EdwinPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: EdwinConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: EdwinConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: EdwinConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: EdwinConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: EdwinConfig): string;
  };
};
```
...
- Introduce `edwin/plugin-sdk`.
- Add `api.runtime` to `EdwinPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `edwinRuntime: ">=2026.2.0"`).
