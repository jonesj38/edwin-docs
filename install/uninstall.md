---
summary: "Uninstall Edwin completely (CLI, service, state, workspace)"
read_when:
  - You want to remove Edwin from a machine
...
Default task name is `Edwin Gateway` (or `Edwin Gateway (<profile>)`).
...
If you used `https://edwinpai.com/install.sh` or `install.ps1`, the CLI was installed with `npm install -g edwin@latest`.
Remove it with `npm rm -g edwin` (or `pnpm remove -g` / `bun remove -g` if you installed that way).
