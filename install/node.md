...
read_when:
  - "You installed Edwin but `edwin` is “command not found”"
...
Edwin’s runtime baseline is **Node 22+**.

If you can run `npm install -g edwin@latest` but later see `edwin: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
