...
OpenCode 完成更改 → 打开 PR → Edwin 审查差异并在 Telegram 中回复"小建议"加上明确的合并决定（包括需要先应用的关键修复）。
...
向"Robby"（@edwin）请求一个本地酒窖 skill。它请求一个示例 CSV 导出 + 存储位置，然后快速构建/测试该 skill（示例中有 962 瓶酒）。
...
跨 Agents、Claude、Codex 和 Edwin 管理 skills/命令的桌面应用。
...
与智能体工作流（Claude Code、Edwin）集成的 Linear CLI。从终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制，然后 Edwin 接管来管理房间空气质量。
...
由屋顶摄像头触发：让 Edwin 在天空看起来很美的时候拍一张照片——它设计了一个 skill 并拍摄了照片。
...
定时提示每天早上通过 Edwin 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/引言）。
</Card>
...
自动化 Todoist 任务，并让 Edwin 直接在 Telegram 聊天中生成 skill。
</Card>
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/edwin-ha-addon">
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/jonesj38/nix-edwin">
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，以 Edwin 作为界面，加上漂亮的 Grafana 仪表板。
