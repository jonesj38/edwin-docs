...
Edwin 可以在隔离的 Docker 容器中运行智能体以确保安全。`sandbox` 命令帮助你管理这些容器，特别是在更新或配置更改后。
...
edwin sandbox list --json     # JSON output
```
...
edwin sandbox recreate --all --force        # Skip confirmation
```
...
edwin sandbox recreate --all
# or just one agent:
edwin sandbox recreate --agent family
```
...
edwin sandbox recreate --agent alfred
```
...
**解决方案：** 使用 `edwin sandbox recreate` 强制移除旧容器。它们会在下次需要时自动使用当前设置重新创建。

提示：优先使用 `edwin sandbox recreate` 而不是手动 `docker rm`。它使用 Gateway 网关的容器命名规则，避免在作用域/会话键更改时出现不匹配。

## 配置

沙箱设置位于 `~/.edwin/edwin.json` 的 `agents.defaults.sandbox` 下（每个智能体的覆盖设置在 `agents.list[].sandbox` 中）：
