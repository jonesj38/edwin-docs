...
你可以在 `~/.edwin/edwin.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/edwin.log"
  }
}
```
...
```bash
edwin channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.edwin/edwin.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/edwin/edwin-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Edwin 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
EDWIN_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `edwin plugins enable diagnostics-otel` 启用插件。
...
- `edwin.tokens`（计数器，属性：`edwin.token`、`edwin.channel`、`edwin.provider`、`edwin.model`）
- `edwin.cost.usd`（计数器，属性：`edwin.channel`、`edwin.provider`、`edwin.model`）
- `edwin.run.duration_ms`（直方图，属性：`edwin.channel`、`edwin.provider`、`edwin.model`）
- `edwin.context.tokens`（直方图，属性：`edwin.context`、`edwin.channel`、`edwin.provider`、`edwin.model`）

消息流：

- `edwin.webhook.received`（计数器，属性：`edwin.channel`、`edwin.webhook`）
- `edwin.webhook.error`（计数器，属性：`edwin.channel`、`edwin.webhook`）
- `edwin.webhook.duration_ms`（直方图，属性：`edwin.channel`、`edwin.webhook`）
- `edwin.message.queued`（计数器，属性：`edwin.channel`、`edwin.source`）
- `edwin.message.processed`（计数器，属性：`edwin.channel`、`edwin.outcome`）
- `edwin.message.duration_ms`（直方图，属性：`edwin.channel`、`edwin.outcome`）

队列 + 会话：

- `edwin.queue.lane.enqueue`（计数器，属性：`edwin.lane`）
- `edwin.queue.lane.dequeue`（计数器，属性：`edwin.lane`）
- `edwin.queue.depth`（直方图，属性：`edwin.lane` 或 `edwin.channel=heartbeat`）
- `edwin.queue.wait_ms`（直方图，属性：`edwin.lane`）
- `edwin.session.state`（计数器，属性：`edwin.state`、`edwin.reason`）
- `edwin.session.stuck`（计数器，属性：`edwin.state`）
- `edwin.session.stuck_age_ms`（直方图，属性：`edwin.state`）
- `edwin.run.attempt`（计数器，属性：`edwin.attempt`）

### 导出的 span（名称 + 关键属性）

- `edwin.model.usage`
  - `edwin.channel`、`edwin.provider`、`edwin.model`
  - `edwin.sessionKey`、`edwin.sessionId`
  - `edwin.tokens.*`（input/output/cache_read/cache_write/total）
- `edwin.webhook.processed`
  - `edwin.channel`、`edwin.webhook`、`edwin.chatId`
- `edwin.webhook.error`
  - `edwin.channel`、`edwin.webhook`、`edwin.chatId`、`edwin.error`
- `edwin.message.processed`
  - `edwin.channel`、`edwin.outcome`、`edwin.chatId`、`edwin.messageId`、`edwin.sessionKey`、`edwin.sessionId`、`edwin.reason`
- `edwin.session.stuck`
  - `edwin.state`、`edwin.ageMs`、`edwin.queueDepth`、`edwin.sessionKey`、`edwin.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `edwin doctor`。
