...
- `~/Library/Application Support/Edwin/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `edwin-canvas://<session>/<path>`

示例：

- `edwin-canvas://main/` → `<canvasRoot>/main/index.html`
- `edwin-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `edwin-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
edwin nodes canvas present --node <id>
edwin nodes canvas navigate --node <id> --url "/"
edwin nodes canvas eval --node <id> --js "document.title"
edwin nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__edwin__/a2ui/
```
...
edwin nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
edwin nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "edwin://agent?message=Review%20this%20design";
```
