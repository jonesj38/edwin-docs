...
Edwin can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Edwin can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Edwin defaults
...
TTS config lives under `messages.tts` in `edwin.json`.
...
- If `provider` is **unset**, Edwin prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.edwin/settings/tts.json`, override with `EDWIN_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Edwin retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Edwin registers
