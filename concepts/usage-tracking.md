...
- `/usage cost` in chats: local cost summary aggregated from Edwin session logs.
- CLI: `edwin status --usage` prints a full per-provider breakdown.
- CLI: `edwin channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
