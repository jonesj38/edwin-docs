...
read_when:
  - You want to understand what “context” means in Edwin
...
“Context” is **everything Edwin sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Edwin-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Edwin builds the system prompt

The system prompt is **Edwin-owned** and rebuilt each run. It includes:
...
By default, Edwin injects a fixed set of workspace files (if present):
