---
summary: "Use Venice AI privacy-focused models in Edwin"
read_when:
  - You want privacy-focused inference in Edwin
...
```bash
edwin onboard --auth-choice venice-api-key
```
...
```bash
edwin chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Edwin shows all available Venice models. Pick based on your needs:
...
```bash
edwin models set venice/claude-opus-45
edwin models set venice/llama-3.3-70b
```

List all available models:

```bash
edwin models list | grep venice
```

## Configure via `edwin configure`

1. Run `edwin configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Edwin automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
edwin chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
edwin chat --model venice/claude-opus-45

# Use uncensored model
edwin chat --model venice/venice-uncensored

# Use vision model with image
edwin chat --model venice/qwen3-vl-235b-a22b

# Use coding model
edwin chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `edwin models list` to see currently available models. Some models may be temporarily offline.
