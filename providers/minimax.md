---
summary: "Use MiniMax M2.1 in Edwin"
read_when:
  - You want MiniMax models in Edwin
...
```bash
edwin plugins enable minimax-portal-auth  # skip if already loaded.
edwin gateway restart  # restart if gateway is already running
edwin onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/jonesj38/edwin/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `edwin.json`:
...
- Use `edwin models list` and `edwin models set minimax/MiniMax-M2.1` to switch.
...
- Running `edwin configure` and selecting **MiniMax M2.1**, or
