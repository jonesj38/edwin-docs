---
summary: "Quick troubleshooting guide for common Edwin failures"
read_when:
...
When Edwin misbehaves, here's how to fix it.
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `edwin status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `edwin status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `edwin status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `edwin gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `edwin channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `edwin gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `edwin logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `edwin status --all` (it redacts tokens). If you paste `edwin status`, consider setting `EDWIN_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  edwin models auth setup-token --provider anthropic
  ```
...
```bash
edwin models auth paste-token --provider anthropic
edwin models status
```
...
- Preferred: `edwin logs --follow`
- File logs (always): `/tmp/edwin/edwin-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$EDWIN_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u edwin-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  edwin config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  edwin config set gateway.mode remote
  edwin config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `edwin setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.edwin/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `edwin doctor`
...
- then run `edwin sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `edwin gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `edwin gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `edwin gateway status` and `edwin doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `EDWIN_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `edwin gateway status` shows a config mismatch**
...
- Fix: rerun `edwin gateway install --force` from the same `--profile` / `EDWIN_STATE_DIR` you want the service to use.

**If `edwin gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `edwin doctor` to update it (or `edwin gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `EDWIN_GATEWAY_TOKEN`) and restart the service.

**If `edwin gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/edwin` on disk.
...
Symptoms: `pwd` or file tools show `~/.edwin/sandboxes/...` even though you
...
Edwin intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `edwin models list` or
  `edwin models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${EDWIN_CONFIG_PATH:-$HOME/.edwin/edwin.json}"
```
...
tail -f "$(ls -t /tmp/edwin/edwin-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
edwin logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.edwin/agents/<agentId>/sessions/
```
...
edwin logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
edwin gateway --verbose
```

If you’re logged out / unlinked:

```bash
edwin channels logout
trash "${EDWIN_STATE_DIR:-$HOME/.edwin}/credentials" # if logout can't cleanly remove everything
edwin channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/edwin/edwin-*.log | head -1)" | tail -20
```

### High Memory Usage

Edwin keeps conversation history in memory.
...
Edwin now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `edwin doctor` reports every invalid entry.
- `edwin doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `edwin logs`, `edwin health`, `edwin status`, `edwin gateway status`, and `edwin gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/edwin/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `edwin models status` (CLI).
...
5. Run `edwin channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Edwin scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/jonesj38/edwin/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
edwin gateway status
edwin gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.edwin.* still works)
```
...
Ensure the global `edwin` CLI is installed and matches the app version:
...
#   ${EDWIN_CONFIG_PATH:-$HOME/.edwin/edwin.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
edwin gateway --verbose
edwin channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                     |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Gateway file logs (structured)    | `/tmp/edwin/edwin-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                        |
| Gateway service logs (supervisor) | macOS: `$EDWIN_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.edwin/logs/...`; profiles use `~/.edwin-<profile>/logs/...`)<br />Linux: `journalctl --user -u edwin-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Edwin Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$EDWIN_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                |
| Media cache                       | `$EDWIN_STATE_DIR/media/`                                                                                                                                                                                                                                                                                    |
| Credentials                       | `$EDWIN_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                              |
...
edwin gateway status --deep

# Is the gateway reachable?
edwin health --json
# If it fails, rerun with connection details:
edwin health --verbose
...
trash "${EDWIN_STATE_DIR:-$HOME/.edwin}"
edwin channels login         # re-pair WhatsApp
edwin gateway restart           # or: edwin gateway
```
...
1. Check logs first: `/tmp/edwin/` (default: `edwin-YYYY-MM-DD.log`, or your configured `logging.file`)
