...
- During heartbeat runs, Edwin treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `edwin sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Edwin skips the heartbeat run entirely (no model call).
...
```bash
edwin system event --text "Check for urgent follow-ups" --mode now
```
