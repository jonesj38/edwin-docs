...
- Service types:
  - `_edwin-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `canvasPort=18793` (default canvas host port; serves `/__edwin__/canvas/`)
  - `cliPath=<path>` (optional; absolute path to a runnable `edwin` entrypoint or binary)
  - `tailnetDns=<magicdns>` (optional hint; auto-detected when Tailscale is available)

Disable/override:

- `EDWIN_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.edwin/edwin.json` controls the Gateway bind mode.
- `EDWIN_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `EDWIN_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `EDWIN_CLI_PATH` overrides the advertised CLI path.
