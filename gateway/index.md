...
- Replaces the legacy `gateway` command. CLI entry point: `edwin gateway`.
...
- Config hot reload watches `~/.edwin/edwin.json` (or `EDWIN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__edwin__/canvas/` from `~/.edwin/workspace/canvas`. Disable with `canvasHost.enabled=false` or `EDWIN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `EDWIN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `EDWIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.edwin.*` may still exist)
- Linux: `edwin-gateway-<profile>.service`
- Windows: `Edwin Gateway (<profile>)`

Install metadata is embedded in the service config:

- `EDWIN_SERVICE_MARKER=edwin`
- `EDWIN_SERVICE_KIND=gateway`
- `EDWIN_SERVICE_VERSION=<version>`
...
```bash
edwin --dev setup
edwin --dev gateway --allow-unconfigured
...
- `EDWIN_STATE_DIR=~/.edwin-dev`
- `EDWIN_CONFIG_PATH=~/.edwin-dev/edwin.json`
- `EDWIN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.edwin/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `EDWIN_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `EDWIN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `EDWIN_CONFIG_PATH`
- unique `EDWIN_STATE_DIR`
...
edwin --profile rescue gateway install
```

Example:

```bash
EDWIN_CONFIG_PATH=~/.edwin/a.json EDWIN_STATE_DIR=~/.edwin-a edwin gateway --port 19001
EDWIN_CONFIG_PATH=~/.edwin/b.json EDWIN_STATE_DIR=~/.edwin-b edwin gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `edwin health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `edwin`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `edwin gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.edwin.*` is cleaned up).
  - `edwin doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Edwin profile services.
  ...
  - Cleanup: `edwin gateway uninstall` (current service) and `edwin doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `edwin gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Edwin.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.edwin.*` labels still unload cleanly).
- To stop it cleanly, use `edwin gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `edwin gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `edwin gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Edwin installs a **systemd user service** by default on Linux/WSL2. We
...
`edwin gateway install` writes the user unit. `edwin doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/edwin-gateway[-<profile>].service`:

```
[Unit]
Description=Edwin Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/edwin gateway --port 18789
Restart=always
RestartSec=5
Environment=EDWIN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now edwin-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/edwin-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now edwin-gateway[-<profile>].service
```
...
- `edwin gateway health|status` — request health/status over the Gateway WS.
- `edwin message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `edwin agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `edwin gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `edwin gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `edwin gateway` and the legacy TCP control port.
