---
summary: "All configuration options for ~/.edwin/edwin.json with examples"
read_when:
...
Edwin reads an optional **JSON5** config from `~/.edwin/edwin.json` (comments + trailing commas allowed).

If the file is missing, Edwin uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.edwin/workspace`). You usually only need a config to:
...
Edwin only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `edwin doctor`, `edwin logs`, `edwin health`, `edwin status`, `edwin service`, `edwin help`).
- Run `edwin doctor` to see the exact issues.
- Run `edwin doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `edwin config set`. Keep a backup of `~/.edwin/edwin.json`.
...
```bash
edwin gateway call config.get --params '{}' # capture payload.hash
edwin gateway call config.apply --params '{
...
```bash
edwin gateway call config.get --params '{}' # capture payload.hash
edwin gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.edwin/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@edwin", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.edwin/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.edwin/workspace" }],
}
```
...
{ "$include": "/etc/edwin/base.json5" } // absolute
...
```json5
// ~/.edwin/clients/mueller/agents.json5
[
...
```json5
// ~/.edwin/clients/mueller/broadcast.json5
{
...
Edwin reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Edwin runs your login shell and imports only the missing expected keys (never overrides).
...
- `EDWIN_LOAD_SHELL_ENV=1`
- `EDWIN_SHELL_ENV_TIMEOUT_MS=15000`
...
Edwin stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.edwin/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.edwin/credentials/oauth.json` (or `$EDWIN_STATE_DIR/credentials/oauth.json`)
...
- `~/.edwin/agent/*` (migrated by `edwin doctor` into `~/.edwin/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `EDWIN_OAUTH_DIR`
- Agent dir (default agent root override): `EDWIN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Edwin imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Edwin derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/edwin/edwin-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/edwin/edwin.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.edwin/credentials/whatsapp/biz
          // authDir: "~/.edwin/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `edwin doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@edwin", "edwin"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@edwin"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.edwin/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.edwin/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.edwin/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.edwin/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.edwin/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.edwin/workspace-home" },
      { id: "work", workspace: "~/.edwin/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `edwin.json`).
...
Edwin starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["edwin-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-edwin",
...
Edwin starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `edwin plugins install @edwin/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Edwin starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Edwin spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Edwin
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/edwin" } },
}
```
...
When a file exceeds this limit, Edwin logs a warning and injects a truncated
...
message envelopes). If unset, Edwin uses the host timezone at runtime.
...
`[edwin]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[edwin]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[edwin]"`, Edwin will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Edwin generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.edwin/settings/tts.json",
      elevenlabs: {
...
Edwin also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Edwin currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `edwin configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:edwin`: all built-in Edwin tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.edwin/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "edwin-sandbox-browser:bookworm-slim",
          containerPrefix: "edwin-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Edwin uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.edwin/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Edwin config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Edwin writes/merges a `models.json` into
`~/.edwin/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Edwin uses
...
- Shortcut: `edwin onboard --auth-choice opencode-zen`.
...
Shortcut: `edwin onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `edwin onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `edwin onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `edwin onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `edwin onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `edwin onboard --auth-choice minimax-api`.
...
- Override the agent config root with `EDWIN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.edwin/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.edwin/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.edwin/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Edwin stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.edwin/skills` (workspace skills
...
from `~/.edwin/extensions`, `<workspace>/.edwin/extensions`, plus any
...
### `browser` (edwin-managed browser)

Edwin can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for edwin and expose a small loopback control service.
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/edwin" }
...
- When a connection comes from one of these IPs, Edwin uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `edwin gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `EDWIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `EDWIN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Edwin
...
The Gateway watches `~/.edwin/edwin.json` (or `EDWIN_CONFIG_PATH`) and applies changes automatically.
...
- `~/.edwin/edwin.json` (or `EDWIN_CONFIG_PATH`)
...
- `EDWIN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `edwin --dev …` → uses `~/.edwin-dev` + shifts ports from base `19001`
- `edwin --profile <name> …` → uses `~/.edwin-<name>` (port via config/env/flags)
...
Gmail helper config (used by `edwin webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "edwin@gmail.com",
...
- Set `EDWIN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Edwin defaults `serve.path` to `/` so
...
Default root: `~/.edwin/workspace/canvas`  
Default port: `18793` (chosen to avoid the edwin browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__edwin__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__edwin__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.edwin/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.edwin/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.edwin/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_edwin-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `edwin` (advertises `edwin.local`). Override with `EDWIN_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_edwin-gw._tcp` under `~/.edwin/dns/` using the configured discovery domain (example: `edwin.internal.`).
