---
summary: "Chrome extension: let Edwin drive your existing Chrome tab"
read_when:
...
The Edwin Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate edwin-managed Chrome profile.
...
Edwin then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Edwin release (npm package) as static files. There is no separate “build” step.

After upgrading Edwin:

- Re-run `edwin browser extension install` to refresh the installed files under your Edwin state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Edwin ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Edwin to control.
...
- `ON`: attached; Edwin can drive that tab.
...
Debugging: `edwin sandbox explain`
...
`edwin browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `edwin browser extension install` first to copy the extension to a stable location under your Edwin state directory.
...
- **This is not isolated** like the dedicated edwin-managed profile.
