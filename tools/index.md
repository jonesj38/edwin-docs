---
summary: "Agent tool surface for Edwin (browser, canvas, nodes, message, cron) replacing legacy `edwin-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `edwin-*` skills
title: "Tools"
---

# Tools (Edwin)

Edwin exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `edwin-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `edwin.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Edwin logs a warning and ignores the allowlist so core tools stay available.
...
- [Pipelines](/tools/pipelines): typed workflow runtime with resumable approvals (requires the Pipelines CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`edwin node run`).
...
- Requires a Brave API key (recommended: `edwin configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Edwin-managed browser.
...
- Quick smoke: `edwin nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `edwin gateway` restart in-place)
