...
Matrix is an open, decentralized messaging protocol. Edwin connects as a Matrix **user**
...
```bash
edwin plugins install @edwin/matrix
```

Local checkout (when running from a git repo):

```bash
edwin plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `edwin plugins install @edwin/matrix`
   - From a local checkout: `edwin plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Edwin calls the same
     login endpoint, stores the access token in `~/.edwin/credentials/matrix/credentials.json`,
...
- On first connection, Edwin requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Edwin logs a warning.
...
`~/.edwin/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Edwin resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
