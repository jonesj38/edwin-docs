...
Tlon is a decentralized messenger built on Urbit. Edwin connects to your Urbit ship and can
...
```bash
edwin plugins install @edwin/tlon
```

Local checkout (when running from a git repo):

```bash
edwin plugins install ./extensions/tlon
```
...
Use these with `edwin message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Edwin replies in-thread.
