...
read_when:
  - Setting up Twitch chat integration for Edwin
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Edwin connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
edwin plugins install @edwin/twitch
```

Local checkout (when running from a git repo):

```bash
edwin plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `EDWIN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "edwin", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use EDWIN_TWITCH_ACCESS_TOKEN env var)
...
```bash
EDWIN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
