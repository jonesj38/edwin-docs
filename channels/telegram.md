...
Edwin can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, Edwin retries the same message as plain text.

## Commands (native + custom)

Edwin registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; Edwin does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `edwin logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). Edwin can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. Edwin:
...
Private chats can include `message_thread_id` in some edge cases. Edwin keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `edwin pairing approve telegram <CODE>`
...
2. Run `edwin logs --follow` and look for `from.id`.
...
Edwin defaults to audio files for backward compatibility.
...
Edwin supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, Edwin handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, Edwin caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.edwin/telegram/sticker-cache.json`
...
Edwin uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, Edwin:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by Edwin)
...
- Example: `edwin message send --channel telegram --target 123456789 --message "hi"`.
...
- `edwin channels status` shows a warning when config expects unmentioned group messages.
- `edwin channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `edwin logs --follow` (look for "skipping group message")
...
- Upgrade to a Edwin build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
