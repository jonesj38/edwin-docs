...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Edwin to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
edwin plugins install @edwin/feishu
```

Local checkout (when running from a git repo):

```bash
edwin plugins install ./extensions/feishu
```
...
If you just installed Edwin, run the wizard:
...
2. Configuring app credentials in Edwin
...
1. You already ran `edwin channels add` for Feishu
2. The gateway is running (`edwin gateway status`)
...
```bash
edwin pairing approve feishu <CODE>
```
...
2. Run `edwin logs --follow` and look for `chat_id`
...
2. Run `edwin logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `edwin gateway status`  | Show gateway status           |
| `edwin gateway install` | Install/start gateway service |
| `edwin gateway stop`    | Stop gateway service          |
| `edwin gateway restart` | Restart gateway service       |
| `edwin logs --follow`   | Tail gateway logs             |
...
4. Check logs: `edwin logs --follow`
...
5. Ensure the gateway is running: `edwin gateway status`
6. Check logs: `edwin logs --follow`
