...
```bash
edwin plugins install @edwin/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
edwin plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Edwin" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `edwin pairing list nextcloud-talk`
  - `edwin pairing approve nextcloud-talk <CODE>`
