...
3. Set tokens for Edwin and start the gateway.
...
7. Slash Commands → create `/edwin` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Edwin can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Edwin falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Edwin",
    "description": "Slack connector for Edwin"
  },
  "features": {
    "bot_user": {
      "display_name": "Edwin",
    ...
    "slash_commands": [
      {
        "command": "/edwin",
        "description": "Send a message to Edwin",
        "should_escape": false
      }
    ]
  },
...
By default, Edwin replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Edwin infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `edwin pairing approve slack <code>`.
...
- On startup, Edwin resolves channel/user names in allowlists to IDs (when tokens allow)
