...
read_when:
  - You want Edwin to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Edwin to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`edwin onboard`) and `edwin channels add` list optional channel plugins.
...
```bash
edwin plugins install @edwin/nostr
```

Use a local checkout (dev workflows):

```bash
edwin plugins install --link <path-to-edwin>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "edwin@example.com",
        "lud16": "edwin@example.com"
      }
    }
  }
}
