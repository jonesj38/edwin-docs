...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/edwin`)
...
If the page fails to load, start the Gateway first: `edwin gateway`.
...
you revoke it with `edwin devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.edwin/edwin.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Edwin
...
Edwin **blocks** Control UI connections without device identity.
