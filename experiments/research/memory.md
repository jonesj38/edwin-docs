---
summary: "Research notes: offline memory system for Edwin workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.edwin/workspace) beyond daily Markdown logs
...
Target: Edwin-style workspace (`agents.defaults.workspace`, default `~/.edwin/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.edwin/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Edwin to see every session.
...
```
~/.edwin/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Edwin**, but keep a separable core library.

### Why integrate into Edwin?
...
- You want the agent itself to call the tools:
  - `edwin memory recall "…" --k 25 --since 30d`
  - `edwin memory reflect --since 7d`
...
Pragmatic take for `~/.edwin/workspace`:
