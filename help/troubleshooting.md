...
You can also set `EDWIN_VERBOSE=1` instead of the flag.
...
### `docs.edwinpai.com` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.edwinpai.com` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.edwinpai.com` to the allowlist, then retry.
...
- Check the allowlist: `edwin config get agents.defaults.models`
...
If you can, include the relevant log tail from `edwin logs --follow`.
