...
- Installs the global CLI (`edwin`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `edwin gateway install`.
...
(or `bot.molt.<profile>` when using `--profile`/`EDWIN_PROFILE`; legacy `com.edwin.*` still unloads).
...
The app registers the `edwin://` URL scheme for local actions.
...
1. Install and launch **Edwin.app**.
