---
summary: "Edwin on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Edwin on a Raspberry Pi
  - Running Edwin on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Edwin on Raspberry Pi

## Goal

Run a persistent, always-on Edwin Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Edwin features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled edwin

# Enable if not
sudo systemctl enable edwin

# Start on boot
sudo systemctl start edwin
```
...
```bash
# Check logs
journalctl -u edwin --no-pager -n 100

# Common fix: rebuild
cd ~/edwin  # if using hackable install
npm run build
sudo systemctl restart edwin
```
