...
- **Remote over SSH (default)**: Edwin commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
1. Install Node + pnpm and build/install the Edwin CLI (`pnpm install && pnpm build && pnpm link --global`).
...
2. Under **Edwin runs**, pick **Remote over SSH** and set:
   ...
   - **CLI path** (advanced): optional path to a runnable `edwin` entrypoint/binary (auto-filled when advertised).
3. Hit **Test remote**. Success indicates the remote `edwin status --json` runs correctly. Failures usually mean PATH/CLI issues; exit 127 means the CLI isn’t found remotely.
...
- Run `edwin channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
- Re-run login on that host if auth expires. Health check will surface link problems.

## Troubleshooting

- **exit 127 / not found**: `edwin` isn’t on PATH for non-login shells. Add it to `/etc/paths`, your shell rc, or symlink into `/usr/local/bin`/`/opt/homebrew/bin`.
- **Health probe failed**: check SSH reachability, PATH, and that Baileys is logged in (`edwin status --json`).
...
Pick sounds per notification from scripts with `edwin` and `node.invoke`, e.g.:

```bash
edwin nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
