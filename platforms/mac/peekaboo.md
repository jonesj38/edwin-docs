...
read_when:
  - Hosting PeekabooBridge in Edwin.app
...
- **Host**: Edwin.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `edwin ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Edwin is a thin broker host.
...
When enabled, Edwin starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Edwin app TeamID).
...
- If `peekaboo` reports “bridge client is not authorized”, ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Edwin.app)
