...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `edwin sandbox explain`.
...
Legacy `agent.*` configs are migrated by `edwin doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=edwin-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${EDWIN_STATE_DIR:-$HOME/.edwin}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
